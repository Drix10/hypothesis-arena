...
| Calibrated answers, exactly 4 questions | JEV sidecar (doc 03) | Size or execute |
| Snapshot, decision table, R1-R17, kills | C++ `risk/` + `kill/` (docs 05, 10) | Call LLMs |
| Orders, stops, journal-before-order | C++ `exec/` + `journal/` (doc 06) | Act without a journal row |
...
- JEV is the only decision engine. Four questions; richness lives in state.
- Research is isolated by OS permissions (R11), not convention.
- No X API, no Selenium, free data only. Absent sentiment is not neutral.
...
5. No automatic stage promotion. 6. Pinned `typesafe/jev-1.13`, never floating.
...
Spec: `plan/00-INDEX.md` … `plan/11-*.md`. Live checklist: `TODO.md`
