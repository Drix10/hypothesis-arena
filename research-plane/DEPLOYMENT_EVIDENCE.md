...
| FRED / ALFRED (+ vintage replay) | same + original-vintage replay | PROVEN 2026-09-23 (`sandbox/fred_vintage_probe.py` + artifact `fred-vintage-evidence.json` + 4 replay-predicate tests): real GDP observations ($32,486.066 @ 2026-04-01, N=3 p50 ~500ms p99 ~844ms); realtime-period replay as-known-2020-01-01 (pre-revision 2019 quarters) byte-identical across two fetches; bad key 400-denied (fail-closed); zero key leakage in artifact. Honest note: dedicated `/alfred/*` paths 404 for this key — the realtime-parameter mechanism on the FRED endpoint is the proven replay path. Same key serves both (one credential). |
...
- Auditor ruling 2026-09-22: `mirohuman` nologin is NOT A BLOCKER — keep
  unchanged. The invariant is that services never execute as the human/
  control-plane operator; a real human login account performs sign-off.
  No script change.
