...
- [ ] P1.4 SOAKING: loop gated on MIRO_CONTACT (MISSING_REQUIRED_CONFIG exit 2). Config-loading inconsistency FIXED: soak_check now uses config.load() (same root-.env resolution as the soak), 5/5 config regression tests, leakage audit clean (UA carries contact, zero artifact hits), 7/7 acceptance green. Still needs: 7-day host for --loop. Loop NOT started. LOOP RUNNING since 2026-09-18T18:54Z. 26h checkpoint 2026-09-19T20:45Z: 106 cycles, 0x403, heartbeats clean, acceptance 7/7, day-2 brought 40 new EDGAR events (books balance: inputs=unique+dupes), pregrade 115/115 auto_genuine 0 noise. Early-exit gate ~green; decision: let ride to 48h, then close P1.4 + freeze evidence. 48h CLOSE SEQUENCE (locked): 1) stop loop 2) freeze/archive evidence 3) complete SOAK_REPORT.md 4) record shortened 48h-not-7d criterion explicitly. Report language (exact): crashes -> "No ingest crashes were observed during the monitored run; cycle-level crash/error counters were not persistently recorded. Record-balance checks showed no unexplained drops." Scope line (exact): "P1.4 validates operational/data-pipeline reliability over the observed 48h window. It does not validate trading alpha, profitability, or the statistical validity of the eventual strategy." Then lifecycle hardening, then Jane Street analysis.
