...
- [x] P3.1 AMENDMENT at `52ccedb` (PENDING human re-sign): P3.3 integration's systematic sweep exposed a soundness bug in the frozen verifier — `ScalarLessL`'s LE L-table wrong from byte 5 on, false-rejecting ~6% of VALID sidecar signatures (never false-accepting). Root-caused by cross-language bisection to the constant itself (canon/hash/sha512/reduction/point-math all proven equal). Fix = the 32-byte constant only, cross-checked vs Python integers + Reduce512 limbs. Full gates re-run green (P3.1 102/102 + P3.2 37/37 + 20k fuzz, normal+hardened). No fixture changed.
- [x] P3.3 DONE technical gate at `f8be0ac` (PENDING human sign-off in doc 07): typed JEVStateV3 (closed schema, serializer bit-equal to frozen sidecar canon incl. tricky strings/mixed scalars, strict FromJVal/CheckStateShape) + KernelState (immutable universe, per-symbol epochs, sole request source) + decision table (doc 03 §3.2 verbatim + budget-dependent gate + max-gate split; tiers only, no sizing). test_p33 68/68 + 200-AnswerSet replay hashed twice identical + non-degenerate + §3.7 21–28 verbatim. v2 cases 1–20 NOT re-encoded (no v3 mapping in plan; invented nothing). P3.4 defaults to (b), gated.
...
- [ ] Later: 11. Phase 2.5 research plane (bundle model, sandbox image) → 12. Phase 3 C++ core cont. (P3.1 AMENDED pending re-sign; P3.2 FROZEN; P3.3 technical gate green pending sign-off; P3.4/P3.5 next) → 13. Phase 4 paper loop G0 → 14-16. G1/G2/G3 gates. (10. Phase 2 JEV sidecar DONE, accepted/frozen `50ea369` + authorized hardening.)
