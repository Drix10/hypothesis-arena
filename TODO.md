...
- [x] PRE-P3.3 HARDENING PASS 2 DONE (review-driven, `465dfd6`/`6669e95`/`bbb8182`/`98cef3e`): collect tuple status/records + strict row types + no truncation/coercion + schedule/cache fail-closed + immediate non-transient return; soak heartbeat_at freshness + UNREADABLE FAIL + exit-code propagation; jev OS-native lock + spend_charge-None HOLD + malformed-spend-name UNKNOWN + atomic log rotation + confidence-null reject + recursive nonfinite scan; ctx dup-keys + bundle allowlist + kind registry + unique feature_id + strict history; classify KNOWN_FIELDS + corrections UNIQUE. New test_collect.py + test_soak_check.py. Full suite + freeze + C++ normal/hardened + 20k fuzz green; kernel/ untouched; evidence pristine. P3.3 STILL NOT AUTHORIZED (awaiting human review).
