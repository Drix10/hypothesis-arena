...
- [x] PRE-P3.3 HARDENING PASS 5 DONE (review-driven, transactional/durability slice: `1028d9b`/`4be2563`/`f8382b0`/`3bbb2be`): JEV ambiguous-POST no-retry/no-refund (reservation stands + governor trips, human reconciles) + in-lock cache recheck single-flight (two-process 1-call proof) + unknown-cost HOLD (no answer on unknowable bill) + shared LIVE/REPLAY artifact validator (signature-before-pins, structured HOLDs) + cache_put/log_row failure containment; collector durable-first ordering (signals append is the commit point, validators/schedule after, ConfigError recovery rule); classify ClassifyAbort on DB/infra (no publish, exit 3) + malformed-signals abort + signals-integrity checker verdict; soak snapshot-at-creation timestamps with explicit evidence-day + soak singleton (exit 3); ctx history-future bound + ingested>=observed; spend future-ledger corruption flag; docs (03 ambiguity/unknown-cost/single-flight, 09 commit ordering, 10 absolute-means-absolute, 13 budget-dependent gate); +46 freeze pins. Full suite + freeze + C++ normal/hardened + 20k fuzz green; kernel/ + P3.2 vectors byte-identical; evidence pristine. P3.3 STILL NOT AUTHORIZED (awaiting human review).
