...
- [x] P1.3 DONE 2026-09-18, HARDENED same day: canonical retrieved_at (not source observed_at), future-timestamp REJECT invariant, pre-ingest schema gate (no KeyError, batch never dies), authoritative-payload hash (observed_at excluded), revision_of populated, EDGAR-only amendment scope, header-anchored item detection, deterministic fresh/stale tests, audit split input_records/unique_event_keys/unique_content_versions. 16 checks pass; 105-baseline re-run identical (105/105/0/0/0/23/40/42/23).
