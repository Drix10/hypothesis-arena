...
- [x] P3.3 CORRECTION at `7d83974` (technical gate green, human sign-off pending in doc 07): human review rejected the first P3.3 state (feature IDs only). Corrected to the EXACT frozen §3.4 contract — full 17-key states + full feature records + exact nested allowlists + single checked-int path + int64-micros freshness + expected_symbol binding + non-copyable/mutex KernelState with try_accept() sole gate + universe cap 5 (manifest) + frozen VetoReason codes. test_p33 78/78 + 200-replay SHA-256 proof (twice identical, non-degenerate) + per-artifact closed loop (C++ bytes hash to sidecar state_hash/decision_key) + §3.7 21–28 verbatim. v2 1–20 NOT re-encoded (no v3 mapping in plan). Sidecar untouched. P3.4 defaults to (b), gated.
...
- [ ] Later: 11. Phase 2.5 research plane (bundle model, sandbox image) → 12. Phase 3 C++ core cont. (P3.1 RE-SIGNED; P3.2 FROZEN; P3.3 correction green pending sign-off; P3.4/P3.5 next) → 13. Phase 4 paper loop G0 → 14-16. G1/G2/G3 gates. (10. Phase 2 JEV sidecar DONE, accepted/frozen `50ea369` + authorized hardening.)
