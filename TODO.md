...
- [x] PRE-P3.3 HARDENING PASS 4 DONE (review-driven, `88bdc38`/`53e8dda`/`c09ee09`/`a5fd10f`/`867948d`): JEV expired-artifact admission (exp>now + created-skew) + full decision-clock propagation (spend/cache/HOLD/created, replay never wall-clock) + frozen $2.00 pre-call reservation with settle/refund/pricing-violation (two-process cap test + single-call overshoot refusal); collector singleton lock (exit 3, cross-process test) + deferred ETag persistence + narrow network exceptions + explicit records contract + full source allowlist + run-instant scheduling; soak MISSED_RANGE restart derivation + iter_poll_rows context-managed reader + malformed-classified integrity + PRAGMA + audit-present (no cross-day fallback) + run_step evidence codes; classify rowid tie-break + published<=observed<=retrieved + DB finally; ctx frozen watermark envelope (exact keys/namespace/skew/coverage) + history namespace + strict monotonicity + symbols<=16. Docs: X HISTORICAL/NON-PRODUCTION (09 table move + 02 banner), 04 intent recipe reconciled to 06, state_hash vs context_hash frozen (03+04), R6/R7 data-age gates, MEDIUM flatten FSM, pooled-Holm family, daily-return Sharpe spec, universe_vN artifact, README FROZEN-DESIGN/IMPLEMENTED/VERIFIED states. Full suite + freeze + C++ normal/hardened + 20k fuzz green; kernel/ + P3.2 vectors byte-identical; evidence pristine. P3.3 STILL NOT AUTHORIZED (awaiting human review).
...
- [ ] Later: 11. Phase 2.5 research plane (bundle model, sandbox image) → 12. Phase 3 C++ core cont. (P3.1/P3.2 FROZEN; pre-P3.3 hardening in progress, then P3.3/P3.4/P3.5) → 13. Phase 4 paper loop G0 → 14-16. G1/G2/G3 gates. (10. Phase 2 JEV sidecar DONE, accepted/frozen `50ea369` + authorized hardening.)
