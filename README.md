<div align="center">
  ...
  **Status: P3.1 ACCEPTED · P3.2 FROZEN · P3.3 ACCEPTED · P3.4 = quarantine · P3.5 ACTIVE (E/F/G ACCEPTED, D NOT AUTHORIZED, pollers EDGAR/FRED/Treasury ACCEPTED, BLS/BEA NEXT, H1 OPEN). Paper only. No live capital.**
...
| Risk/sizing/execution (P3.5) | ACTIVE: E (STAGE verify) + F (feed gaps) + G (Snapshot v1) ACCEPTED; D NOT AUTHORIZED; H1 router/journal/reconcile = the remaining build |
| Research plane, paper loop (G0) | PARTIAL: 6-node graph + spend/ledger control plane built, 250-test hosted battery green with WAE (108 graph + 116 hardening + 19 emit + 7 seam-graph); evidence job 205 green. Status words — PROVEN (probe/evidence proves the tested capability) vs WIRED (a production caller consumes it) vs EXECUTION-READY (H1 order lifecycle exists + drilled): Track A production seam (Runner→Seam→graph→publisher→ctx) ACCEPTED (independent audit, 6fdeda7); Tier-A adapters ACCEPTED + hosted-green (EDGAR 47, FRED 24, Treasury 23, BLS 21, BEA 23 tests) with live soak/p50-p99 evidence OPEN; BEA/Alpaca-read/earnings-gate PROVEN as probes; H1 execution OPEN, so the loop is NOT execution-ready. OANDA practice BLOCKED (India ineligible) — stocks-only G0 on Alpaca paper. 7-day proof OPEN (see TODO.md Phase-D gate) |
